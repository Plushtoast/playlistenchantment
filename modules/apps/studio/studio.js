...
export class Studio extends HandlebarsApplicationMixin(ApplicationV2) {
    ...
    static PARTS = {
        ...
        browser: {
            ...
            templates: [`${TEMPLATES}/folder.hbs`, `${TEMPLATES}/entry.hbs`],
            scrollable: [".pe-browser-body"],
        },
        content: {
            template: `${TEMPLATES}/content.hbs`,
            templates: [`${TEMPLATES}/track.hbs`, `${TEMPLATES}/pad.hbs`, `${TEMPLATES}/settings.hbs`],
            scrollable: [".pe-content-body"],
        },
        rail: { template: `${TEMPLATES}/rail.hbs`, scrollable: [".pe-queue-list"] },
    ...
    showView(view) {
        ...
        Settings.set("studioState", this.#state);
        if (this.rendered) this.render();
        return this;
    }
    ...
    #prepareBrowser(context) {
        const tree = Librarian.tree({ boards: this.isBoardView, ...this.#filter });
    ...
    #prepareTracks(context) {
        ...
        const searching = !!this.#query || this.#tags.size > 0 || this.#channels.size > 0;
    ...
    #prepareBoard(context) {
        ...
        context.pads = SoundboardService.visiblePads(board).map((sound) => this.#padContext(sound));
    ...
    #prepareSettings(context) {
        ...
        context.hotkey = Settings.studioBindingLabel();
        context.capturing = this.#capturingHotkey;
        const conflicts = Settings.conflictingActions(Settings.studioBinding());
        context.hotkeyConflict = conflicts.length
            ? game.i18n.format("KEYBINDINGS.Conflict", { conflicts: game.i18n.getListFormatter().format(conflicts) })
            : "";
    ...
    #trackContext(sound, { showSource = false } = {}) {
        ...
        return {
            ...
            color: TrackMeta.colorFor(sound),
            tags: Tags.decorate(TrackMeta.tagsFor(sound)).map((tag) => ({ ...tag, inherited: !ownTags.includes(tag.tag) })),
            playing: sound.playing,
            paused: !sound.playing && !!sound.pausedTime,
            loading: sound.playing && !sound.sound?.loaded,
            repeat: sound.repeat,
    ...
    #padContext(sound) {
        ...
        return {
            ...
            color: meta.color || TrackMeta.read(sound.parent).color,
            playing: sound.playing,
            loop: config.loop,
            duck: config.duck,
    ...
    async _onRender(context, options) {
        ...
        this.element.classList.toggle("pe-view-settings", this.isSettingsView);
        this.element.classList.toggle("pe-compact", this.isCompact);
    ...
    #tick() {
        ...
        if (!current) {
            if (bar.dataset.soundId) this.render({ parts: ["transport", "rail"] });
        ...
        if (bar.dataset.soundId !== current.id || bar.dataset.playing !== String(current.playing)) {
            this.render({ parts: ["transport", "rail"] });
        }
    }
    ...
    #debouncedSearch = foundry.utils.debounce((value) => {
        ...
        this.element?.querySelector('[data-action="clearFilters"]')?.toggleAttribute("disabled", !this.#query && !this.#tags.size);
    }, 250);
    ...
    #onPointerDown(event) {
        ...
        if (!handle || this.isSettingsView || this.isCompact) return;
    ...
    #clampBrowserWidth(width, content = this.element?.querySelector(".window-content")) {
        const available = (content?.clientWidth ?? 0) - Studio.CONTENT_WIDTH_MIN - Studio.RAIL_WIDTH;
        const max = Math.max(Studio.BROWSER_WIDTH_MIN, available);
        return Math.round(Math.clamp(width, Studio.BROWSER_WIDTH_MIN, max));
    }

    #applyBrowserWidth() {
        const content = this.element?.querySelector(".window-content");
        if (!content) return;
        const width = this.#clampBrowserWidth(this.#state.browserWidth ?? Studio.BROWSER_WIDTH_DEFAULT, content);
        content.style.setProperty("--pe-browser-width", `${width}px`);
    }
    ...
    #taggableFrom(element) {
        const target = this.#soundFrom(element) ?? this.#playlistFrom(element);
        if (!target) return null;
        return Permissions.canEdit(target.documentName === "Playlist" ? target : target.parent) ? target : null;
    }
    ...
    #onDragOver(event) {
        ...
        if (this.#isTagDrag(event)) {
            const row = event.target.closest("[data-sound-id], [data-playlist-id]");
    ...
    static _onSelectPlaylist(_event, target) {
        ...
        if (this.isBoardView) this.#state.boardId = id;
        else this.#state.playlistId = id;
        Settings.set("studioState", this.#state);
        this.render({ parts: ["browser", "content"] });
    }
    ...
    static async _onToggleCompact() {
        ...
        if (compact) this.#state.expandedSize = { width: this.position.width, height: this.position.height };
    ...
    static async _onNewPlaylist(event, target) {
        ...
        if (!name) return;
        const playlist = await Librarian.createPlaylist({ name, folder: this.#folderIdFrom(target) });
        if (playlist) {
    ...
    static async _onNewFolder(event, target) {
        event.stopPropagation();
        const name = await this.#promptName("PLAYLISTENCHANTMENT.LIBRARY.newFolder");
        if (!name) return;
        await Librarian.createFolder({ name, parent: this.#folderIdFrom(target) });
        this.render({ parts: ["browser"] });
    }
    ...
    static _onCaptureHotkey() {
        ...
        const onKey = async (event) => {
            ...
            if (!binding) return;
            await Settings.setStudioBinding(binding);
            this.#stopHotkeyCapture();
        };
