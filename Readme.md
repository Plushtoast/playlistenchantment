...
```html
https://raw.githubusercontent.com/Plushtoast/playlistenchantment/master/module.json'
```

## Features

* Currently played tracks also now have a background color for better visibility
* Tracks can be started with a default volume other than 50% by default. This will not apply to soundboards. Also it'll overwrite any existing volume settings for that sound.
* Tracks can have a default fade in time. It will overwrite any existing fade time on playlists.
* New buttons to pause/start/forward/rewind all currently played tracks
...
* Drag and drop sound folders to run random playlists
* Preview next sound
